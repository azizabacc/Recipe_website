<?php
session_start();

//include_once($rootPath.'includes/mysql.php');
require_once($rootPath.'includes/mysql.php');
include_once($rootPath.'config/user.php');
include_once($rootPath.'includes/variables.php');
include_once($rootPath.'includes/functions.php');

$postData = $_POST;

if (
    !isset($postData['comment']) &&
    !isset($postData['recipe_id']) &&
    !isset($postData['review']) &&
    !is_numeric($postData['review']) &&
    !is_numeric($postData['recipe_id'])
    )
{
	echo('Le commentaire est invalide.');
    return;
}

if (!isset($loggedUser)) {
    echo('Vous devez être authentifié pour soumettre un commentaire');
    return;
}

$comment = $postData['comment'];
$recipeId = $postData['recipe_id'];
$review = $postData['review'];

$insertRecipe = $mysqlClient->prepare('INSERT INTO comments(comment, recipe_id, user_id, review) VALUES (:comment, :recipe_id, :user_id, :review)');
$insertRecipe->execute([
    'comment' => $comment,
    'recipe_id' => $recipeId,
    'user_id' => retrieve_id_from_user_mail($loggedUser['email'], $users),
    'review' => (int) $review,
]);

?>

<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>comment recipes</title>
    <link
        href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" 
        rel="stylesheet"
    >
</head>
<body class="d-flex flex-column min-vh-100">
    <div class="container">

    <?php include_once($rootPath.'includes/header.php'); ?>
        <h1>Comment added successfully!</h1>
        
        <div class="card">
            
            <div class="card-body">
                <p class="card-text"><b>Note</b> : <?php echo($review); ?> / 5</p>
                <p class="card-text"><b>Your comment</b> : <?php echo strip_tags($comment); ?></p>
            </div>
        </div>
    </div>
    <?php include_once($rootPath.'includes/footer.php'); ?>
</body>
</html>
